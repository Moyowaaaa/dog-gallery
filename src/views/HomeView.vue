...
const searching = () => {
  const searchedImages = computed(() => {
    return dogImages.value.filter((dogImage:any) => {
      return dogImage.toLowerCase().includes(search.value.toLowerCase())
    })
  })
  console.log(searchedImages.value)
  searchedItem.value = (searchedImages.value)
}
...
<template>
  ...
  <div class="min-h-screen w-full   text-2xl">
    ...
    <div class="flex justify-center">
      <input type="text" class="border-2 border-[red] p-2" placeholder="Search Images...." v-model="search" @input="searching"/>
    </div>
